...
namespace DawnOfLight.Base.Network
{
    public class UPnPNat
    {
        ...
        private static XmlDocument MakeSOAPRequest(string url, string soap, string function)
        {
        ...
        public List<PortForwarding> ListForwardedPort()
        {
            ...
            var forwadedPort = new List<PortForwarding>();
            for (int index = 0;; ++index)
            {
                try
                {
                    XmlDocument xdoc = MakeSOAPRequest(
                        _serviceUrl,
            ...
            if (internalIp == null || internalIp.ToString() == IPAddress.Any.ToString())
            {
                internalIp =
                    Dns.GetHostAddresses(Dns.GetHostName())
                        .Where(ip => ip.GetAddressBytes().Length == 4)
                        .FirstOrDefault();
