...
namespace DawnOfLight.GameServer.commands.Player
{
	...
	public class MakeLeaderCommandHandler : ICommandHandler
	{
		public void OnCommand(GameClient client, string[] args)
		{
			...
			{
				client.Out.SendMessage("You are not part of a group.",ChatType.CT_System,ChatLocation.CL_SystemWindow);
				return;
			}
			...
			else //Setting by name
			{
				...
				{ // Invalid target
					client.Out.SendMessage("No players in group with that name.",ChatType.CT_System,ChatLocation.CL_SystemWindow);
					return;
				}
				if(target==client.Player)
				{
					client.Out.SendMessage("You are the group leader already.",ChatType.CT_System,ChatLocation.CL_SystemWindow);
					return;
				}
			...
			client.Player.Group.SendMessageToGroupMembers(target.Name + " is new group leader.", ChatType.CT_System, ChatLocation.CL_SystemWindow);
		}
	}
}