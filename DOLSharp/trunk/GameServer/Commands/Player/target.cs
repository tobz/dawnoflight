		...
		public void OnCommand(GameClient client, string[] args)
		{
			...
			if (args.Length == 2)
			{
				...
				if (targetClient != null)
				{
					...
					client.Out.SendMessage("You target " + targetPlayer.GetName(0, true) + ".", ChatType.CT_System, ChatLocation.CL_SystemWindow);
					return;
				}
				if (client.Account.PrivLevel > 1)
				{
					...
					while (en.MoveNext())
					{
						if (((GameObject)en.Current).Name == args[1])
						{
							...
							client.Out.SendMessage("[GM] You target " + ((GameObject)en.Current).GetName(0, true) + ".", ChatType.CT_System, ChatLocation.CL_SystemWindow);
							return;
						}
					}
				}
			...
			if (client.Account.PrivLevel > 1)
			{
				client.Out.SendMessage("/target <player/mobname>", ChatType.CT_System, ChatLocation.CL_SystemWindow);
			}
			else
			{
				client.Out.SendMessage("/target <playername>", ChatType.CT_System, ChatLocation.CL_SystemWindow);
			}
		}
	}
}