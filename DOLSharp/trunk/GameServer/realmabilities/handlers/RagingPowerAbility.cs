...
namespace DawnOfLight.GameServer.RealmAbilities.handlers
{
	...
	public class RagingPowerAbility : TimedRealmAbility
	{
		...
		public override void Execute(GameLiving living)
		{
			...
			if (player != null)
			{
				if (healed > 0) player.Out.SendMessage("You gain " + healed + " mana.", ChatType.CT_Spell, ChatLocation.CL_SystemWindow);
				if (heal > healed)
				{
					player.Out.SendMessage("You have full mana.", ChatType.CT_Spell, ChatLocation.CL_SystemWindow);
				}
			}
