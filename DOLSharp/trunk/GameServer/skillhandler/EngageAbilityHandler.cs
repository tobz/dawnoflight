...
namespace DawnOfLight.GameServer.SkillHandler
{
	...
	public class EngageAbilityHandler : IAbilityActionHandler
	{
		...
		public void Execute(Ability ab, GamePlayer player)
		{
			...
			if (!player.IsAlive)
			{
                player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Engage.CannotUseDead"), ChatType.CT_YouHit, ChatLocation.CL_SystemWindow);
                return;
			}

			if (player.IsSitting)
			{
                player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Engage.CannotUseStanding"), ChatType.CT_YouHit, ChatLocation.CL_SystemWindow);
                return;
			}

			if (player.ActiveWeaponSlot == GameLiving.eActiveWeaponSlot.Distance)
			{
                player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Engage.CannotUseNoCaCWeapons"), ChatType.CT_YouHit, ChatLocation.CL_SystemWindow);
                return;
			}
			...
			if (target == null)
			{
                player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Engage.NoTarget"), ChatType.CT_System, ChatLocation.CL_SystemWindow);
                return;
			}
			...
			{
                player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Engage.TargetAttackedRecently", target.GetName(0, true)), ChatType.CT_System, ChatLocation.CL_SystemWindow);
                return;
			}

			if (!GameServer.ServerRules.IsAllowedToAttack(player, target, true))
			{
                player.Out.SendMessage(LanguageMgr.GetTranslation(player.Client.Account.Language, "Skill.Ability.Engage.NotAllowedToEngageTarget", target.GetName(0, false)), ChatType.CT_System, ChatLocation.CL_SystemWindow);
                return;
			}
			//Cancel old engage effects on player
